...
{
    public partial class frmFinishGoodScanIn_v1o4 : Form
    {
        ...
        public frmFinishGoodScanIn_v1o4()
        {
            InitializeComponent();
        ...
        private void frmFinishGoodScanIn_v1o4_Load(object sender, EventArgs e)
        {
        ...
        public void fnPackPRO(string pack)
        {
            ...
            bool packStd = false;
            string packStd_PCS = "", packStd_BOX = "", packStd_CAR = "", packStd_PAL = "";
            int _qty = 0, _packStd_PCS=0, _packStd_BOX = 0, _packStd_CAR = 0, _packStd_PAL = 0;
                ...
                if (packKind == "PCS" || packKind == "BOX" || packKind == "CAR" || packKind == "PAL")
                {
                    ...
                    switch (packKind)
                    {
                        case "PCS":
                            ...
                            packStd_PCS = _partPCS;
                            _packStd_PCS = (packStd_PCS.Length > 0) ? Convert.ToInt32(packStd_PCS) : 0;
                            packStd = (_qty == _packStd_PCS) ? true : false;
                            break;
                        case "BOX":
                            sync = (_partBOX != "0") ? true : false;
                            qty = txtIN_BOX.Text.Trim();
                            _qty = (qty.Length > 0) ? Convert.ToInt32(qty) : 0;
                            packStd_BOX = _partBOX;
                            _packStd_BOX = (packStd_BOX.Length > 0) ? Convert.ToInt32(packStd_BOX) : 0;
                            packStd = (_qty == _packStd_BOX) ? true : false;
                            break;
                        case "CAR":
                            sync = (_partCAR != "0") ? true : false;
                            qty = txtIN_CAR.Text.Trim();
                            _qty = (qty.Length > 0) ? Convert.ToInt32(qty) : 0;
                            packStd_CAR = _partCAR;
                            _packStd_CAR = (packStd_CAR.Length > 0) ? Convert.ToInt32(packStd_CAR) : 0;
                            packStd = (_qty == _packStd_CAR) ? true : false;
                            break;
                        case "PAL":
                            sync = (_partPAL != "0") ? true : false;
                            qty = txtIN_PAL.Text.Trim();
                            _qty = (qty.Length > 0) ? Convert.ToInt32(qty) : 0;
                            packStd_PAL = _partPAL;
                            _packStd_PAL = (packStd_PAL.Length > 0) ? Convert.ToInt32(packStd_PAL) : 0;
                            packStd = (_qty == _packStd_PAL) ? true : false;
                            break;
                    }
                            ...
                            if (ds.Tables[0].Rows.Count == 0)
                            {
                                if (qty != "0")
                                {
                                    ...
                                    if (partName.ToLower().Contains("dispenser"))
                                    {
                                        ...
                                        frmFinishGoodScanIn_v1o4_CheckProof frm = new frmFinishGoodScanIn_v1o4_CheckProof(packStd);
                                        frm.fnGetPart(boxCode, partIDx, partName, partCode, partLOT, partDAY, partLOC, partQTY);
                                        //frm.fnGetPart(boxCode, partIDx, partName, partCode, partLOT, partDAY, partLOC, partQTY, packStd);
                                        frm.ShowDialog();
        ...
        private void lnk_ChangeQty_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            uc_Dispenser_Item_Remain_Scan_V1o1_ProBox pro_box = new uc_Dispenser_Item_Remain_Scan_V1o1_ProBox();
            pro_box.ShowDialog();
        }
